...
#e. water age tracking in 5 soil layers, groundwater, and ET

Lumped_VSA_model_Mod_Ages <-
function(
  ...
  Bexp=2, #exponent
	...
	SoilWater_a_Age<-vector(length=length(P))
	SoilWater_b_Age<-vector(length=length(P))
	SoilWater_c_Age<-vector(length=length(P))
	SoilWater_d_Age<-vector(length=length(P))
	SoilWater_e_Age<-vector(length=length(P))
	GW_Age <-vector(length=length(P))
	ET_Age<-vector(length=length(P))
	ET_a_Age<-vector(length=length(P))
	ET_b_Age<-vector(length=length(P))
	ET_c_Age<-vector(length=length(P))
	ET_d_Age<-vector(length=length(P))
	ET_e_Age<-vector(length=length(P))
	ET_GW_Age <-vector(length=length(P))
	...
	for(i in (2):length(deltaP)){  
		...
		SoilWater_a_Age[i] = SoilWater_a_Age[i - 1]
		ET_a_Age[i] = SoilWater_a_Age[i]
		} 
		
		#3.2: Wetting below Theta_fc
		else if (deltaP[i] + SoilWater_a[i-1] <= AWC_a){
		SoilWater_a[i]<- deltaP[i] + SoilWater_a[i-1]
		SoilWater_a_Age[i] = (SoilWater_a[i-1]*SoilWater_a_Age[i - 1] + deltaP[i]*1)/(SoilWater_a[i])
		ET_a_Age[i] = SoilWater_a_Age[i]
		}
		...
		SoilWater_a_Age[i] = (SoilWater_a[i-1]*SoilWater_a_Age[i - 1] + deltaP[i]*1)/(deltaP[i] + SoilWater_a[i-1])
		ET_a_Age[i] = SoilWater_a_Age[i]
		}
		...
		ET_b[i] <- max(0, SoilWater_b[i-1] - SoilWater_b[i])
		SoilWater_b_Age[i] = SoilWater_b_Age[i - 1]
		ET_b_Age[i] = SoilWater_b_Age[i]
		}
		...
		SoilWater_b_Age[i] = ((SoilWater_b[i-1] - wz_b_prime[i])*SoilWater_b_Age[i - 1] + excess_a[i]*SoilWater_a_Age[i - 1])/(excess_a[i] + SoilWater_b[i-1])
		ET_b_Age[i] = SoilWater_b_Age[i]		
		}
		...
		SoilWater_b_Age[i] = (SoilWater_b[i-1]*SoilWater_b_Age[i-1] + excess_a[i]*SoilWater_a_Age[i])/(SoilWater_b[i-1] + excess_a[i])
		ET_b_Age[i] = SoilWater_b_Age[i]
		}
		...
		ET_c[i] <- max(0, SoilWater_c[i-1] - SoilWater_c[i])		
		SoilWater_c_Age[i] = SoilWater_c_Age[i - 1]
		ET_c_Age[i] = SoilWater_c_Age[i]
		}
		#4.2: Wetting below Theta_fc
		else if ((excess_b[i] - wz_c_prime[i]) + SoilWater_c[i-1] <= AWC_c){
		SoilWater_c[i]<- (excess_b[i] - wz_c_prime[i]) + SoilWater_c[i-1]
		SoilWater_c_Age[i] = ((SoilWater_c[i-1] - wz_c_prime[i])*SoilWater_c_Age[i - 1] + excess_b[i]*SoilWater_b_Age[i - 1])/(excess_b[i] + SoilWater_c[i-1])
		ET_c_Age[i] = SoilWater_c_Age[i]		
		}
		#4.3: Wetting above Theta_fc
		else { 
		...
		SoilWater_c_Age[i] = (SoilWater_c[i-1]*SoilWater_c_Age[i-1] + excess_b[i]*SoilWater_b_Age[i])/(SoilWater_c[i-1] + excess_b[i])
		ET_c_Age[i] = SoilWater_c_Age[i]
		}		
		...
		ET_d[i] <- max(0, SoilWater_d[i-1] - SoilWater_d[i])
 		SoilWater_d_Age[i] = SoilWater_d_Age[i - 1]
		ET_d_Age[i] = SoilWater_d_Age[i]
		}		
		
		#4.2: Wetting below Theta_fc
		else if ((excess_c[i] - wz_d_prime[i]) + SoilWater_d[i-1] <= AWC_d){
		SoilWater_d[i]<- (excess_c[i] - wz_d_prime[i]) + SoilWater_d[i-1]
		SoilWater_d_Age[i] = ((SoilWater_d[i-1] - wz_d_prime[i])*SoilWater_d_Age[i - 1] + excess_c[i]*SoilWater_c_Age[i - 1])/(excess_c[i] + SoilWater_d[i-1])
		ET_d_Age[i] = SoilWater_d_Age[i]		
		}
		#4.3: Wetting above Theta_fc
		else { 
		...
		SoilWater_d_Age[i] = (SoilWater_d[i-1]*SoilWater_d_Age[i-1] + excess_c[i]*SoilWater_c_Age[i])/(SoilWater_d[i-1] + excess_c[i])
		ET_d_Age[i] = SoilWater_d_Age[i]
		}				
		...
		ET_e[i] <- max(0, SoilWater_e[i-1] - SoilWater_e[i])
 		SoilWater_e_Age[i] = SoilWater_e_Age[i - 1]
		ET_e_Age[i] = SoilWater_e_Age[i]
		}			
		
		#4.2: Wetting below Theta_fc
		else if ((excess_d[i] - wz_e_prime[i]) + SoilWater_e[i-1] <= AWC_e){
		SoilWater_e[i]<- (excess_d[i] - wz_e_prime[i]) + SoilWater_e[i-1]
		SoilWater_e_Age[i] = ((SoilWater_e[i-1] - wz_e_prime[i])*SoilWater_e_Age[i - 1] + excess_d[i]*SoilWater_d_Age[i - 1])/(excess_d[i] + SoilWater_e[i-1])
		ET_e_Age[i] = SoilWater_e_Age[i]		
		}		
		...
		SoilWater_e_Age[i] = (SoilWater_e[i-1]*SoilWater_e_Age[i-1] + excess_d[i]*SoilWater_d_Age[i])/(SoilWater_e[i-1] + excess_d[i])
		ET_e_Age[i] = SoilWater_e_Age[i]
		}		
		...
		if (TM_S[i] > 0)
		{GW_Age[i] = (TM_S[i-1]*GW_Age[i-1] + excess_e[i]*SoilWater_e_Age[i] )/(TM_S[i-1] + excess_e[i])}
		else
		{GW_Age[i] = 0}
		
		ET_GW_Age[i] = GW_Age[i]
		...
		#Total ET Age
		if (ET[i] > 0)
			{ET_Age[i] = (ET_a[i]*ET_a_Age[i] + ET_b[i]*ET_b_Age[i] + ET_c[i]*ET_c_Age[i] + ET_d[i]*ET_d_Age[i] + ET_e[i]*ET_e_Age[i])/ET[i]}
		else
			{ET_Age[i] = 0}
		#All stores age by 1 day
		SoilWater_a_Age[i] <- SoilWater_a_Age[i] + 1
		SoilWater_b_Age[i] <- SoilWater_b_Age[i] + 1
		SoilWater_c_Age[i] <- SoilWater_c_Age[i] + 1
		SoilWater_d_Age[i] <- SoilWater_d_Age[i] + 1
  ...
  #Not currently used (10/21/19 JK)
	...
	rain_snowmelt<-P
	Streamflow<-data.frame(Date=as.Date(as.character(dateSeries)),  Tmax, Tmin, rain_snowmelt, modeled_flow, baseflow, deltaP, Pe, Se, Ia, SoilWater_a, SoilWater_b, SoilWater_c, SoilWater_d, SoilWater_e, PET, wz_a_prime, wz_b_prime, wz_c_prime, wz_d_prime, wz_e_prime, wz_GW, ET, ET_a, ET_b, ET_c, ET_d, ET_e, ET_GW, TM_S, totQ, SoilWater_a_Age, SoilWater_b_Age, SoilWater_c_Age, SoilWater_d_Age, SoilWater_e_Age, GW_Age, ET_a_Age, ET_b_Age, ET_c_Age, ET_d_Age, ET_e_Age, ET_Age, Rainfall)[OutStart:length(P),] 
	return(Streamflow)
}